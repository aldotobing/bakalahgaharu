...
import { Star, Eye, X, Share2, Facebook, Twitter, Linkedin, Link as LinkIcon } from 'lucide-react';
...
const ProductCard: React.FC<ProductCardProps> = ({ product, t, isRTL, currentLanguage }) => {
  ...
  const shareProduct = async (platform?: string) => {
    ...
    const shareText = `${t.products.productDetails.share}: ${product.name} - ${product.grade} ${t.products.grade} ${t.products.productDetails.availability.toLowerCase()} ${product.price}${product.currency}/kg`;
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {productImages.map((img, index) => (
                        <button
                          key={index}
                          className={`aspect-square rounded-lg overflow-hidden border-2 transition-colors ${selectedImageIndex === index ? 'border-amber-400' : 'border-transparent'}`}
                  ...
                  <div>
                    ...
                    <div className="space-y-6">
                      <div className="bg-amber-50 p-4 rounded-lg">
                        <h4 className="font-semibold text-amber-800 mb-2">{t.products.productDetails.description}</h4>
                        <p className="text-gray-700 text-sm leading-relaxed">{product.description}</p>
                      </div>
                      
                      {/* Product Details */}
                      <div className="bg-white border border-gray-100 rounded-lg p-4 shadow-sm">
                        <h4 className="font-semibold text-gray-800 mb-3">{t.products.productDetails.details}</h4>
                        <ul className="space-y-2 text-sm text-gray-600">
                          <li className="flex justify-between">
                            <span className="text-gray-500">{t.products.productDetails.productId}:</span>
                            <span className="font-medium">{product.id}</span>
                          </li>
                          <li className="flex justify-between">
                            <span className="text-gray-500">{t.products.productDetails.availability}:</span>
                            <span className="text-green-600 font-medium">{t.products.productDetails.inStock}</span>
                          </li>
                          <li className="flex justify-between">
                            <span className="text-gray-500">{t.products.productDetails.shipping}:</span>
                            <span className="font-medium">{t.products.productDetails.worldwide}</span>
                          </li>
                          <li className="flex justify-between">
                            <span className="text-gray-500">{t.products.productDetails.leadTime}:</span>
                            <span className="font-medium">3-5 {t.products.productDetails.businessDays}</span>
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        <h4 className="font-semibold text-gray-800 mb-3">{t.products.productDetails.share}</h4>
                        <div className="flex space-x-2">
                          <button 
                    ...
                    <div className="space-y-4 mb-6">
                      <div className="flex items-center justify-between pb-2 border-b border-gray-100">
                        <span className="text-gray-600 font-medium">{t.products.grade}:</span>
                        <span className="font-medium text-gray-800">{product.grade}</span>
                      </div>
                      <div className="flex items-start justify-between pb-2 border-b border-gray-100">
                        <span className="text-gray-600 font-medium mt-1">{t.products.colors}:</span>
                        <div className="flex flex-wrap gap-2 justify-end">
                          {product.colors.map((color, index) => (
                            <span key={index} className="text-sm bg-amber-100 text-amber-800 px-3 py-1 rounded-full">
                              {color}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        <span className="text-2xl font-bold text-amber-600">
                          ${product.price.toLocaleString()} <span className="text-base font-normal text-gray-500">/ kg</span>
                        </span>
                      </div>
                    </div>
